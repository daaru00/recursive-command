...
module.exports = class Executor {
  ...
  async executeSequentially(command, cwds) {
    for (let index = 0; index < cwds.length; index++) {
      await this.execute(command, cwds[index] || process.cwd(), `[${index.toLocaleString('en-US', {minimumIntegerDigits: 2})}] `)
    }
  }
  ...
  async executeParallel(command, cwds) {
    ...
    for (let index = 0; index < cwds.length; index++) {
      promises.push(this.execute(command, cwds[index] || process.cwd(), `[${index.toLocaleString('en-US', {minimumIntegerDigits: 2})}] `))
    }
