...
module.exports = class Executor {
  ...
  async execute(command, cwd, tag) {
    ...
    return new Promise((resolve, reject) => {
      ...
      commandExecution.stderr.on('data', function (data) {
        data.toString().split(os.EOL).forEach(line => consola.error({
          type: tag,
          message: line + ' '
        }))
      })
  ...
  async executeSequentially(command, cwds) {
    for (let index = 0; index < cwds.length; index++) {
      await this.execute(command, cwds[index] || __dirname, index.toString())
    }
  }
  ...
  async executeParallel(command, cwds) {
    ...
    for (let index = 0; index < cwds.length; index++) {
      promises.push(this.execute(command, cwds[index] || __dirname, index.toString()))
    }
